                    ...
                    <div class="row row-cols-2 row-cols-md-3 row-cols-lg-4">
                        ...
                        >
                            <div class="image-wrapper-gallery">
                                <img
                                    :src="image.file_path"
                                    :style="image.order_number === 1 ? 'border: 2px red solid' : ''"
                                    alt="Product image"
                                />
...
export default {
    ...
    methods: {
        ...
        getProductImages() {
            axios
                .get(route("models.images.index", this.product), { params: { model_name:  'Product' } })
                .then((response) => {
                    this.max_order = response.data.max_order;
                    this.productImages = response.data.images;
                    this.form.model = 'Product';
                    this.form.currObjID = this.product.id;
                });
        },
        submitForm() {
            ...
            message = "Product gallery has been updated.";
        ...
        deleteProductImage(image) {
            confirmAction().then(() => {
                axios
                    .delete(route("models.images.destroy", [this.product.id, image, 'Product']))
                    .then((response) => {
    ...
    mounted() {
        ...
        Bus.$on("open-product-images-modal", (product) => {
            this.form.model = "Product";
            this.form.currObjID = product.id;
