<template>
    <div
        ...
        tabindex="-1"
        role="dialog"
        aria-labelledby="Team member images modal"
        aria-hidden="true"
        ref="modal"
    >
...
export default {
    ...
    computed: {
        title() {
            return this.teamImages?.length
                ? "Edit team member gallery"
                : "Add image to team member gallery";
        },
        imageLinks() {
            return this.teamImages.map((image) => image.file_path);
        },
    },
    watch: {
        slide() {
    ...
    methods: {
        ...
        getTeamImages() {
            axios
                .get(route("models.images.index", this.team), { params: { model_name:  'Team' } })
                .then((response) => {
                    this.max_order = response.data.max_order;
                    this.teamImages = response.data.images;
                    this.form.model = 'Team';
                    this.form.currObjID = this.team.id;
                });
        },
        submitForm() {
            let method, path, message;
            method = "post";
            path = route("models.images.store", this.team);
            message = "Team member gallery has been updated.";
        ...
        deleteTeamImage(image) {
            confirmAction().then(() => {
                axios
                    .delete(route("models.images.destroy", [this.team.id, image, 'Company']))
                    .then((response) => {
    ...
    mounted() {
        ...
        Bus.$on("open-team-images-modal", (team) => {
            this.form.model = "Team";
            this.form.currObjID = team.id;
            this.team = team;
            if (this.team) {
                this.getTeamImages();
            }
